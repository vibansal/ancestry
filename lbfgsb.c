...
#include "lbfgsb.h"
#include "Lbfgsb.3.0/blas.c"
#include "Lbfgsb.3.0/linpack.c"
#include "Lbfgsb.3.0/timer.c"

//http://www.ece.northwestern.edu/~nocedal/Software/lbfgs_um.tar.gz | code for unbounded version

// this version also works for unbounded case: set nbd[i] = 0
...
{
        int i=0; for (i=0;i<sl;i++) s1[i] = s2[i];
        return sl;
}

double machine_eps ()
{
            dbl_64 s; double value = 1.0;
                s.d64 = value;
                s.i64++;
                return s.d64 - value;
...
double optim_LBFGS_BOUNDED(int numvariables, double *initvector, double (*function)( double x[]), void (*dfunction)( double x[], double deriv[]),double *lowerbounds, double *upperbounds,int *nbd, int iprint,int* negative_delta)
{
	...
	double likelihood,prevld; int iter=0; int extraiter=0; *negative_delta =0;
	...
	csave, logical *lsave, integer *isave, doublereal *dsave)
{
    ...
    /* Builtin functions */
//    integer s_cmp(char *, char *, ftnlen, ftnlen);
    ///* Subroutine */ int new_stringcopy(char *, char *, ftnlen, ftnlen);
//    integer f_open(olist *), s_wsfe(cilist *), do_fio(integer *, char *, ftnlen), e_wsfe(void);
    ...
    extern /* Subroutine */ int prn1lb_(integer *, integer *, doublereal *, 
	    ...
	    *, integer *, char *, integer *, integer *, doublereal *, 
	    doublereal *), prn3lb_(integer *, doublereal *, 
...
/*     Compute f0 and g0. */
    new_stringcopy(task, "FG_START", 8);
...
/*     Compute the infinity norm of the (-) projected gradient. */
    ...
    if (*iprint >= 1) {
	fprintf(stdout,"At iteration %i f=%f\t|proj g| = %e\n", iter, *f, sbgnrm);
	/*
	s_wsfe(&io___55);
...
/*                                terminate the algorithm. */
	new_stringcopy(task, "CONVERGENCE: NORM_OF_PROJECTED_GRADIENT_<=_PGTOL",48);
	goto L999;
    }
...
/*         singular triangular system detected; refresh the lbfgs memory. */
	if (*iprint >= 1) {
		fprintf(stdout,"singular triangular system detected; refresh the lbfgs memory and restart the iteration\n");
	   // s_wsfe(&io___59);
	   // e_wsfe();
	}
	info = 0;
...
/*          refresh the lbfgs memory and restart the iteration. */
	if (*iprint >= 1) {
		fprintf(stdout,"nonpositive definiteness in Cholesky factorization; refresh the lbfgs memory and restart the iteration.\n");
	   // s_wsfe(&io___61);
	   // e_wsfe();
	}
	info = 0;
...
/*                restore the actual number of f and g evaluations etc. */
	    ...
	    new_stringcopy(task, "ABNORMAL_TERMINATION_IN_LNSRCH",30);
	    ++iter;
	    goto L999;
	} else {
/*             refresh the lbfgs memory and restart the iteration. */
	    ...
	    iupdat = 0;
	    updatd = FALSE_;
	    new_stringcopy(task, "RESTART_FROM_LNSRCH",19);
	    timer_(&cpu2);
...
/*                            skip the L-BFGS update. */
	...
	updatd = FALSE_;
	if (*iprint >= 1) {
		fprintf(stdout,"ys=%f, gs=%f, L-BFGS-B update SKIPPED\n", dr, ddum);
	   // s_wsfe(&io___68);
...
/* L20: */
	...
	if (*prjctd) {
	    ...
	    //s_wsle(&io___74);
	    //do_lio(&c__9, &c__1, "The initial X is infeasible.  Restart with its projection.", (ftnlen)58);
	    //e_wsle();
	}
	if (! (*cnstnd)) {
...
/*       the derivative f1 and the vector p = W'd (for theta = 1). */
	...
	if (*iprint >= 0) {
	   ...
	   // s_wsle(&io___81); do_lio(&c__9, &c__1, "Subgnorm = 0.  GCP = X.", (ftnlen)23); e_wsle();
	}
    ...
    col2 = *col << 1;
    f1 = 0.;
    if (*iprint >= 99) { fprintf(stdout,"-----CAUCHY ENTERED ------------\n");
	//s_wsfe(&io___89);e_wsfe();
    }
...
/*           (if iter=2, initialize heap). */
    ...
    dt = tj - tj0;
    if (dt != 0. && *iprint >= 100) {
	fprintf(stdout,"Piece %i --f1, f2 at start point %e %e\n",*nseg,f1,f2);
	fprintf(stdout,"Distance to the next break point: %e\n",dt);
	fprintf(stdout,"Distance to the stationary point: %e\n",dtm);
	/*
	s_wsfe(&io___110);
...
/*       reset the corresponding component of d to zero. */
    ...
    if (*iprint >= 100) {
	fprintf(stdout,"Variable %i is fixed \n",ibp); 
	/*
	s_wsle(&io___115);
...
/*     Check the input arguments for errors. */
    ...
    /* Function Body */
    if (*n <= 0) {
	new_stringcopy(task, "ERROR: N .LE. 0",15);
    }
    if (*m <= 0) {
	new_stringcopy(task, "ERROR: M .LE. 0",15);
    }
    if (*factr < 0.) {
	new_stringcopy(task, "ERROR: FACTR .LT. 0",19);
    }
...
/*                                                   return */
	    new_stringcopy(task, "ERROR: INVALID NBD",18);
	    *info = -6;
	    *k = i__;
	}
	if (nbd[i__] == 2) {
	    if (l[i__] > u[i__]) {
/*                                    return */
		new_stringcopy(task, "ERROR: NO FEASIBLE SOLUTION",27);
		*info = -7;
		*k = i__;
	    }
	}
...
/*            write(6,*) ' index = ', i */
		...
		indx2[*ileave] = k;
		if (*iprint >= 100) {
		    fprintf(stdout,"Variable %i leaves the set of free variables\n",k);
			/*
		    s_wsle(&io___162);
...
/* L20: */
	...
	for (i__ = *nfree + 1; i__ <= i__1; ++i__) {
		...
		indx2[*nenter] = k;
		if (*iprint >= 100) {
		    fprintf(stdout,"Variable %i enters the set of free variables\n",k);
			/*
		    s_wsle(&io___163);
		    do_lio(&c__9, &c__1, "Variable ", (ftnlen)9);
		    do_lio(&c__3, &c__1, (char *)&k, (ftnlen)sizeof(integer));
...
/* L22: */
	...
	if (*iprint >= 99) {
	    i__1 = *n + 1 - *ileave;
		fprintf(stdout,"%i variables leave; %i variables enter\n",i__1, *nenter);	
	/*
	    s_wsle(&io___164);
...
/* L24: */
    ...
    if (*iprint >= 99) {
	fprintf(stdout," %i variables are free at GCP %i\n", *nfree, i__1);
	/*
	s_wsle(&io___166);
...
/*                               Line search is impossible. */
		fprintf(stdout," ascent direction in projection gd = \n");
		/*
	    s_wsle(&io___177);
    ...
    *xstep = *stp * *dnorm;
	...
	new_stringcopy(task, "FG_LNSRCH",9);
	++(*ifun);
	++(*nfgv);
	*iback = *ifun - 1;
	if (*stp == 1.) {
	...
	doublereal *epsmch)
{
    ...
    /* Builtin functions */
//    integer s_wsfe(cilist *), do_fio(integer *, char *, ftnlen), e_wsfe(void),    s_wsle(cilist *), do_lio(integer *, integer *, char *, ftnlen), 	    e_wsle(void);
	...
	xstep)
{
    ...
    /* Builtin functions */
 //   /* Subroutine */ int new_stringcopy(char *, char *, ftnlen, ftnlen);
//    integer s_wsle(cilist *), do_lio(integer *, integer *, char *, ftnlen),  e_wsle(void), s_wsfe(cilist *), do_fio(integer *, char *, ftnlen),    e_wsfe(void);
...
/*                            the subspace minimization converged. */
	new_stringcopy(word, "con",3);
    } else if (*iword == 1) {
/*                          the subspace minimization stopped at a bound. */
	new_stringcopy(word, "bnd",3);
    } else if (*iword == 5) {
/*                             the truncated Newton step has been used. */
	new_stringcopy(word, "TNT",3);
    } else {
	new_stringcopy(word, "---",3);
    }
    if (*iprint >= 99) {
	fprintf(stdout,"LINE SEARCH %i times; norm of step = %e\n",*iback, *xstep);
	/*
	s_wsle(&io___189);
	...
	e_wsle();
	*/
	fprintf(stdout,"At iterate %i f=%e |proj g| = %e\n",*iter,*f,*sbgnrm); 
	/*
	s_wsfe(&io___190);
	do_fio(&c__1, (char *)&(*iter), (ftnlen)sizeof(integer));
	do_fio(&c__1, (char *)&(*f), (ftnlen)sizeof(doublereal));
	...
	*/
	if (*iprint > 100) {
	    fprintf(stdout,"\nX=");
	    //s_wsfe(&io___191);  do_fio(&c__1, "X =", (ftnlen)3);
	    i__1 = *n;
	    for (i__ = 1; i__ <= i__1; ++i__) {
		fprintf(stdout,"\t%e",x[i__]); 
		//do_fio(&c__1, (char *)&x[i__], (ftnlen)sizeof(doublereal));
	    } fprintf(stdout,"\n");
		/*
	    e_wsfe();
...
/* Subroutine */ int prn3lb_(integer *n, doublereal *x, doublereal *f, char *
	...
	doublereal *cachyt, doublereal *sbtime, doublereal *lnscht)
{
    /* Format strings */
...
L999:
	...
	if (*info != 0) {
		...
		//s_wsfe(&io___204); e_wsfe();
	    }
	    if (*info == -2) {
		fprintf(stdout,"Matrix in 2nd Cholesky factorization in formk is not Pos. Def.\n");
		//s_wsfe(&io___205);
		//e_wsfe();
	    }
	    if (*info == -3) {
		fprintf(stdout,"Matrix in 3rd Cholesky factorization in formk is not Pos. Def.\n");
		//s_wsfe(&io___206);
		//e_wsfe();
	    }
	    if (*info == -4) {
	    ...
	    if (*info == -5) {
		fprintf(stdout,"Warning:  more than 10 function and gradient evaluations in the last line search.  Termination may possibly be caused by a bad search direction\n");
		//s_wsfe(&io___208);
		//e_wsfe();
	    }
	    if (*info == -6) {
		fprintf(stdout,"Input nbd(%i) is invalid\n", *k);
		/*
		s_wsle(&io___209);
		...
		*/
	    ...
	    if (*info == -7) {
		fprintf(stdout," l(%i) > u(%i). No feasible solutions.\n",*k, *k);
		/*
		s_wsle(&io___210);
		...
		*/
	    ...
	    if (*info == -9) {
		fprintf(stdout,"Line search cannot locate an adequate point after 20 function and gradient evaluations.  Previous x, f and g restored.  Possible causes: 1 error in function or gradient evaluation; 2 rounding error dominate computation.\n");
		//s_wsfe(&io___212);
		//e_wsfe();
	    }
	}
	if (*iprint >= 1) {
		fprintf(stdout,"Cauchy time %.0f seconds.\nSubspace minimization time %.0f seconds.\nLine search time %.0f seconds.\n", *cachyt, *sbtime, *lnscht);
		/*
	    s_wsfe(&io___213);
	...
	fprintf(stdout,"Total User time %.0f seconds.\n", *time);
	/*
	s_wsfe(&io___214);
...
/* L55: */
	...
	fprintf(stdout,"Positive dir derivative in projection \n");
	//s_wsle(&io___241);
	//do_lio(&c__9, &c__1, " Positive dir derivative in projection ", (ftnlen)39);
	//e_wsle();

	fprintf(stdout,"Using the backtracking step \n"); 
	//s_wsle(&io___242);
...
/*        Check the input arguments for errors. */
	if (*stp < *stpmin) {
	    new_stringcopy(task, "ERROR: STP .LT. STPMIN",22);
	}
	if (*stp > *stpmax) {
	    new_stringcopy(task, "ERROR: STP .GT. STPMAX",22);
	}
	if (*g >= 0.) {
	    new_stringcopy(task, "ERROR: INITIAL G .GE. ZERO",26);
	}
	if (*ftol < 0.) {
	    new_stringcopy(task, "ERROR: FTOL .LT. ZERO", 21);
	}
	if (*gtol < 0.) {
	    new_stringcopy(task, "ERROR: GTOL .LT. ZERO", 21);
	}
	if (*xtol < 0.) {
	    new_stringcopy(task, "ERROR: XTOL .LT. ZERO", 21);
	}
	if (*stpmin < 0.) {
	    new_stringcopy(task, "ERROR: STPMIN .LT. ZERO",23);
	}
	if (*stpmax < *stpmin) {
	    new_stringcopy(task, "ERROR: STPMAX .LT. STPMIN",25);
	}
...
/*     Test for warnings. */
	...
	new_stringcopy(task, "WARNING: ROUNDING ERRORS PREVENT PROGRESS", 41);
    }
    if (brackt && stmax - stmin <= *xtol * stmax) {
	new_stringcopy(task, "WARNING: XTOL TEST SATISFIED",28);
    }
    if (*stp == *stpmax && *f <= ftest && *g <= gtest) {
	new_stringcopy(task, "WARNING: STP = STPMAX",21);
    }
    if (*stp == *stpmin && (*f > ftest || *g >= gtest)) {
	new_stringcopy(task, "WARNING: STP = STPMIN",21);
    }
/*     Test for convergence. */
    if (*f <= ftest && abs(*g) <= *gtol * (-ginit)) {
	new_stringcopy(task, "CONVERGENCE",11);
    }
/*     Test for termination. */
